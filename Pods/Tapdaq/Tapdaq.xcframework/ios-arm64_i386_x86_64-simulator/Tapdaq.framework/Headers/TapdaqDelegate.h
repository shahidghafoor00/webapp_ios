...
@optional
/**
 * Called when the SDK successfully received config and at least one ad network is ready to serve ads.
 */
- (void)didLoadConfig;

/**
 * Called when the SDK could not receive config or when config has been received but no ad networks are ready to serve ads.
 *
 * @param error TDError that contains a reason for the failure.
 * @see TDError
 */
 ...
 */
- (void)didLoadBanner
__deprecated_msg("Use -loadBannerForPlacementTag:withSize: and TDBannerAdRequestDelegate instead.");
